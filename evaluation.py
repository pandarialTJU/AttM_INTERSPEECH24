...
def compute_nist_eer(sc_file,
                       output_file,
                       printout=True):
    import pandas as pd
    df_sc = pd.read_csv(sc_file, sep='\t', header=None, names=['file', 'class_label', 'scores'])
    y_true = (df_sc['class_label'] == 'bonafide').values
    y_pred = df_sc['scores'].astype(float).values

    tgt_sc = y_pred[y_true]
    nontgt_sc = y_pred[np.invert(y_true)]
    eer, th = compute_eer(target_scores=tgt_sc, nontarget_scores=nontgt_sc)

    if printout:
        with open(output_file, "w") as f_res:
            f_res.write('\nCM SYSTEM\n')
            f_res.write('\tEER\t\t= {:8.9f} %, Threshold: {:8.9f} '
                        '(Equal error rate for countermeasure)\n'.format(
                            eer * 100, th))
        os.system(f"cat {output_file}")
    return eer * 100, th
